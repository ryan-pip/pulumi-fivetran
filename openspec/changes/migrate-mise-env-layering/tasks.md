...
- [ ] 4.3 Trigger `upgrade-provider.yml` via `workflow_dispatch` — `Setup mise` passes, `upgrade-provider` runs to completion (or opens its PR), and the `go.work` sync step still behaves.
- [ ] 4.4 A pull-request run stays green (base set still covers golangci-lint, build, SDKs).
- [ ] 4.5 Confirm the pattern matches `pulumi-astronomer`'s equivalent change.
